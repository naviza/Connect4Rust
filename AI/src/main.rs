    ...
    while self::Game::get_winner(&b).is_none() {
        ...
        if b.reds_move() {
            match strategy.choose_move(&mut b) {
                Some(m) => {
                    println!("Red piece in column {}", m.col + 1);
                    m.apply(&mut b)
                }
                None => break,
            }
            ...
            let col = line.trim().parse::<u8>();
            if col.is_ok() {
                let m = Place { col: col.unwrap() };

                m.apply(&mut b);
            } else {
                println!("Couldnt read input, please input again");
                continue;
            }
        }
        s = 1 - s;
    }
    println!("{:?}", self::Game::get_winner(&b).unwrap());
    println!("{}", b);
}
