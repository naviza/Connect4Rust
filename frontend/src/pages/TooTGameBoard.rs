...
impl Component for TooTGameBoard {
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            ...
            TooTGameBoardMsg::Clear(_) => {return true;}
            TooTGameBoardMsg::DoNothing => {},
            TooTGameBoardMsg::TestClick => {log::info!("TestClick")},
...
fn toot_render_cell(gamepiece: char, ctx:   &Context<TooTGameBoard>, column: u8) -> Html {
    ...
    if gamepiece == '1' {
        return html! {
            <td
                onclick={ctx.link().callback(move |_event: MouseEvent| TooTGameBoardMsg::SubmitTurn(column, 't'))}
