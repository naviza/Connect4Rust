...
/*
	Find functions
*/
pub fn find_node(tree: &mut RedBlackTree, t: u32) -> RedBlackTree {
	match tree {
		Some(root) => {
...
/*
	Red Black Tree Functions
*/
pub fn print_as_list(tree: &RedBlackTree) {
	match tree {
		Some(node) => {
			print_as_list(&node.borrow().left);
			print!("{},", node.borrow().data);
			print_as_list(&node.borrow().right);
		}
		None => {},
	}
}
